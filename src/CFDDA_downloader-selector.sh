...
for Y in $(seq 1985 2005) ;do
    for M in $months ;do
        ...
        if [ -e /media/stoop/DataFiles/CFDDA/merge/cfdda_merge_$Y$M.nc ]; then
            ...
            for D in $days ;do
                for H in $hours ;do
                    if [ -e /media/stoop/DataStager/TEMP/CFDDA/vars/cfdda_vars_$Y$M$D$H.nc ]; then
                        ...
                        cdo select,name=T2C,V10,U10,swdown cfdda_$Y$M$D$H.v2.nc /media/stoop/DataStager/TEMP/CFDDA/vars/cfdda_vars_$Y$M$D$H.nc &&
            ...
            cdo -b 32 mergetime /media/stoop/DataStager/TEMP/CFDDA/vars/cfdda_vars_$Y$M*.nc /media/stoop/DataFiles/CFDDA/merge/cfdda_merge_$Y$M.nc &&
            #Now delete previous
            rm /media/stoop/DataStager/TEMP/CFDDA/vars/cfdda_vars_$Y$M*.nc
            }
        fi 
    done
done
