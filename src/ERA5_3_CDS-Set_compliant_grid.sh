...
dir_out="/media/DataStager1/TEMP/ERA5/base/"
#dir_out="/media/DataDrive/ERA5/ERA5-EU_BASE/"

# The variables to run over
var='fsr' #'ssrd t2m wspd100m wspd fdif fdir mpsl d2m rh fal fsr'
years='1980 1981 1982 1983 1984 1985 1986 1987 1988 1989 1990 1991 1992 1993 1994 1995 1996 1997 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018' #'1979 1980 1981 1982 1983 1984 1985 1986 1987 1988 1989 1990 1991 1992 1993 1994 1995 1996 1997 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019'
...
do
    ...
    do
        ...
        echo "Rename lat, lon"
        ncrename -d longitude,lon -d latitude,lat -v longitude,lon -v latitude,lat ${dir_in}${fname} ${dir_in}tmpERA1.nc

        # Reorder dimensions (with a normal latitude
        echo "Reorder the dimenstions"
        ncpdq -a time,-lat,lon ${dir_in}tmpERA1.nc ${dir_in}tmpERA12.nc

        # Set the grid to the N320 Grid with CF conventions
        echo "Set the grid to the defined one"
        cdo -setgrid,${dir_in}template-EU.nc ${dir_in}tmpERA12.nc ${dir_in}tmpERAf_${V}.nc
       
        # Rename the attributes and set the variables attributes
        ncatted -O -a map_area,global,o,c,"Europe" -h ${dir_in}tmpERAf_${V}.nc
        ncatted -O -a experiment,global,o,c,"Model: ERA5" -h ${dir_in}tmpERAf_${V}.nc
        ncatted -O -a author,global,a,c,"; Laurens Stoop, UU/KNMI/TenneT" -h ${dir_in}tmpERAf_${V}.nc
        ncatted -O -a history,global,d,, -h ${dir_in}tmpERAf_${V}.nc

        # Setting the new file name and directory
        ncks -h -A ${dir_in}tmpERAf_${V}.nc ${dir_out}${fname}
